...
class UNet_CBAM_ResSE(nn.Module):
    ...
    def forward(self, x):
        # 下采样部分
        R1 = self.conv_1(x)
        R2 = self.conv_2(self.downSample_1(R1 + self.CBAM1(R1)))
        R3 = self.conv_3(self.downSample_2(R2 + self.CBAM2(R2)))
        R4 = self.conv_4(self.downSample_3(R3 + self.CBAM3(R3)))

        Y1 = self.A5(self.downSample_4(R4 + self.CBAM4(R4)))

        # 上采样部分,拼接
        O1 = self.upSample_Conv_1(self.upSample_1(Y1, R4 + self.SE4(R4)))
        O2 = self.upSample_Conv_2(self.upSample_2(O1, R3 + self.SE3(R3)))
        O3 = self.upSample_Conv_3(self.upSample_3(O2, R2 + self.SE2(R2)))
        O4 = self.upSample_Conv_4(self.upSample_4(O3, R1 + self.SE1(R1)))
